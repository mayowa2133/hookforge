...
const EnvSchema = z.object({
  ...
  DESCRIPT_PLUS_MIN_DESKTOP_CRASH_FREE_PCT: z.coerce.number().min(0).max(100).default(99.5),
  DESCRIPT_PARITY_BASELINE_DATE: z.string().regex(/^\d{4}-\d{2}-\d{2}$/).default("2026-02-26"),
  DESCRIPT_PARITY_STREAK_DAYS_REQUIRED: z.coerce.number().int().min(1).max(90).default(30),
  DESCRIPT_PARITY_DIFF_MAX_AGE_DAYS: z.coerce.number().int().min(1).max(120).default(35),
  DESCRIPT_PARITY_DOGFOOD_MIN_DAYS: z.coerce.number().int().min(1).max(60).default(14),
  DESCRIPT_PARITY_DOGFOOD_MIN_SESSIONS: z.coerce.number().int().min(0).max(10000).default(10),
  DESCRIPT_PARITY_PILOT_MIN_SESSIONS: z.coerce.number().int().min(0).max(10000).default(5),
  DESCRIPT_PARITY_REQUIRED_QUALITY_CAPABILITIES: z.string().default("asr,translation,dubbing,lipsync,chat_edit,ai_edit"),
  DESKTOP_UPDATE_DEFAULT_CHANNEL: z.enum(["stable", "beta", "canary"]).default("stable"),
  DESKTOP_APP_ID: z.string().min(1).default("dev.hookforge.desktop"),
  DESKTOP_RELEASES_BASE_URL: z.string().url().default("https://downloads.hookforge.dev/desktop"),
  DESKTOP_RELEASE_SIGNING_SALT: z.string().min(8).default("hookforge-desktop-signing"),
  ...
  DEEPGRAM_API_BASE_URL: z.string().url().optional(),
  ELEVENLABS_API_KEY: z.string().optional(),
  ELEVENLABS_API_BASE_URL: z.string().url().optional(),
  ELEVENLABS_VOICE_CLONE_BASE_URL: z.string().url().optional(),
  LIPSYNC_API_KEY: z.string().optional(),
  LIPSYNC_API_URL: z.string().url().optional(),
  GENERATIVE_MEDIA_API_KEY: z.string().optional(),
  GENERATIVE_MEDIA_API_URL: z.string().url().optional(),
  MUSIC_SFX_API_URL: z.string().url().optional(),
  PROVIDER_HTTP_TIMEOUT_MS: z.coerce.number().int().min(1000).max(120000).default(20000),
  ALLOW_MOCK_PROVIDERS: booleanFromEnv(false),
...
export const env = EnvSchema.parse({
  ...
  DESCRIPT_PLUS_MIN_DESKTOP_CRASH_FREE_PCT: process.env.DESCRIPT_PLUS_MIN_DESKTOP_CRASH_FREE_PCT,
  DESCRIPT_PARITY_BASELINE_DATE: process.env.DESCRIPT_PARITY_BASELINE_DATE,
  DESCRIPT_PARITY_STREAK_DAYS_REQUIRED: process.env.DESCRIPT_PARITY_STREAK_DAYS_REQUIRED,
  DESCRIPT_PARITY_DIFF_MAX_AGE_DAYS: process.env.DESCRIPT_PARITY_DIFF_MAX_AGE_DAYS,
  DESCRIPT_PARITY_DOGFOOD_MIN_DAYS: process.env.DESCRIPT_PARITY_DOGFOOD_MIN_DAYS,
  DESCRIPT_PARITY_DOGFOOD_MIN_SESSIONS: process.env.DESCRIPT_PARITY_DOGFOOD_MIN_SESSIONS,
  DESCRIPT_PARITY_PILOT_MIN_SESSIONS: process.env.DESCRIPT_PARITY_PILOT_MIN_SESSIONS,
  DESCRIPT_PARITY_REQUIRED_QUALITY_CAPABILITIES: process.env.DESCRIPT_PARITY_REQUIRED_QUALITY_CAPABILITIES,
  DESKTOP_UPDATE_DEFAULT_CHANNEL: process.env.DESKTOP_UPDATE_DEFAULT_CHANNEL,
  DESKTOP_APP_ID: process.env.DESKTOP_APP_ID,
  DESKTOP_RELEASES_BASE_URL: process.env.DESKTOP_RELEASES_BASE_URL,
  DESKTOP_RELEASE_SIGNING_SALT: process.env.DESKTOP_RELEASE_SIGNING_SALT,
  ...
  DEEPGRAM_API_BASE_URL: process.env.DEEPGRAM_API_BASE_URL,
  ELEVENLABS_API_KEY: process.env.ELEVENLABS_API_KEY,
  ELEVENLABS_API_BASE_URL: process.env.ELEVENLABS_API_BASE_URL,
  ELEVENLABS_VOICE_CLONE_BASE_URL: process.env.ELEVENLABS_VOICE_CLONE_BASE_URL,
  LIPSYNC_API_KEY: process.env.LIPSYNC_API_KEY,
  LIPSYNC_API_URL: process.env.LIPSYNC_API_URL,
  GENERATIVE_MEDIA_API_KEY: process.env.GENERATIVE_MEDIA_API_KEY,
  GENERATIVE_MEDIA_API_URL: process.env.GENERATIVE_MEDIA_API_URL,
  MUSIC_SFX_API_URL: process.env.MUSIC_SFX_API_URL,
  PROVIDER_HTTP_TIMEOUT_MS: process.env.PROVIDER_HTTP_TIMEOUT_MS,
